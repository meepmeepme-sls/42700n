...
vex::motor intake1 = vex::motor(vex::PORT2);
...
vex::motor tilter = vex::motor(vex::PORT20);
vex::pot potentiometer = vex::pot(Brain.ThreeWirePort.G);
...
float Kp_t = 1.05;
float Kd_t = 1.2;
float PDpower_t = P_t + D_t;
float lastError_t = error_t;

float liftHighPos = 700;
float lift_sixCubes_pos = 915;
...
float tilterPos_high = 885;
float tilterPos_low = 940;
float tilterPos = - tilter.rotation(rotationUnits::deg);
float tilterPos_release_preparation_low = 880;
float tilterPos_release_preparation_high = 600;
...
int main() {
  ...
  while(1) {
    ...
    if(liftToPosition_sixCubes == true){
      ...
      Motor1.rotateTo(1140, rotationUnits::deg, 90, velocityUnits::pct, false);
      Motor2.rotateTo(-1140, rotationUnits::deg, 90, velocityUnits::pct, false);
      
      //tilter pid, doesn't work
      tilterPos = potentiometer.angle(rotationUnits::deg);
      ...
      tilter.spin(directionType::fwd, 2 * (145 - tilterPos), velocityUnits::pct);
    ...
    /*else if(liftToPosition_ground == true){
      ...
      Motor1.rotateTo(100, rotationUnits::deg, 60, velocityUnits::pct, false);
      Motor2.rotateTo(-100, rotationUnits::deg, -60, velocityUnits::pct, false);

      //step2: tilter rotate to position
      tilterPos = - tilter.rotation(rotationUnits::deg);
      tilter.rotateTo(tilterPos_release_preparation_low, rotationUnits::deg, 0.1 * (tilterPos_low - tilterPos), velocityUnits::pct, false);
    ...
    // macro 4, gorund stack release
      ...
      if(Bumper_ == false){
        tilter.rotateTo(tilterPos_low, rotationUnits::deg, 70, velocityUnits::pct);
        vex::task::sleep(200);
    ...
    //normal control code
        ...
        //L1 and L2 are for tower intake
          ...
          intake2.spin(directionType::rev, 50, velocityUnits::pct);
        }
        else if ( controller2.ButtonL2.pressing() ) {
          intake1.spin(directionType::rev, 50, velocityUnits::pct);
        ...
        if(controller1.Axis2.value() != 0){
          ...
          tilter.spin(directionType::fwd, (1.4*(167 - tilterPos) + 8), velocityUnits::pct);
        }
        else if(controller1.ButtonA.pressing()){

          setPoint_t = 194;
          error_t = setPoint_t - potentiometer.angle(degrees);
          derivative_t = error_t - lastError_t;
          P_t = error_t * Kp_t;
          D_t = derivative_t * Kd_t;
          PDpower_t = P_t + D_t + 5;
          ...
          tilter.spin(directionType::fwd, PDpower_t, velocityUnits::pct);
          if(fabs(potentiometer.angle(degrees) - 170) == 1){
            tilter.stop(brakeType::brake);
            vex::task::sleep(300);
          }
          if(potentiometer.angle(degrees) >= 192){
            tilter.stop(brakeType::hold);
          }

      Brain.Screen.printAt(10, 20, "time = %1.2f", Brain.Timer.value());
      Brain.Screen.printAt(10, 40, "speed = %1.2f", tilter.velocity(percent));
      Brain.Screen.printAt(10, 60, "pot = %1.2f", potentiometer.angle(degrees));

      Brain.Screen.drawPixel((Brain.Timer.value() * 50), (272 - tilter.velocity(percent) * 2));

      vex::task::sleep(10);
    }
    else if(controller1.ButtonX.pressing() && (potentiometer.angle(degrees) <= 160)){
      tilter.spin(directionType::fwd, (1.5 * (160 - potentiometer.angle(degrees) + 5)), velocityUnits::pct);
      Brain.Screen.printAt(10, 20, "time = %1.2f", Brain.Timer.value());
      Brain.Screen.printAt(10, 40, "speed = %1.2f", tilter.velocity(percent));
      Brain.Screen.printAt(10, 60, "pot = %1.2f", potentiometer.angle(degrees));

      Brain.Screen.drawPixel((Brain.Timer.value() * 10), (272 - tilter.velocity(percent) * 2));  
    }
      ...
      tilter.stop(brakeType::hold);
        ...
        Brain.Screen.printAt(10, 160, "pot = %1.2f", potentiometer.angle(rotationUnits::deg));
        Brain.Screen.printAt(10, 200, "tilter = %1.2f", tilter.rotation(rotationUnits::deg));
